...
def preprocess(anchor, target, blockSize):
    ...
    if isinstance(anchor, str) and isinstance(target, str):
        anchorFrame = ((cv2.cvtColor(cv2.imread(anchor), cv2.COLOR_BGR2YCrCb))[50:110, 100:220])[:, :, 0] # get luma component
        targetFrame = ((cv2.cvtColor(cv2.imread(target), cv2.COLOR_BGR2YCrCb))[50:110, 100:220])[:, :, 0] # get luma component
